...
class Emsdk(Package):
    ...
    def _patch_line_regex(self, varname):
        return re.compile(r"^{} = (.*)$".format(varname), flags=re.MULTILINE)
    ...
    def _patch_emscripten_config_contents(self, file_contents):
        ...
        file_contents = self._patch_emscripten_config_line("NODE_JS", node_path, file_contents)
        # Use the directory containing the `lld` binary.
        lld_path = os.path.dirname(str(which("lld")))
        file_contents = self._patch_emscripten_config_line("LLVM_ROOT", lld_path, file_contents)
    ...
    _clear_env_var_line = re.compile(
        r"^Clearing existing environment variable: (.*)$", flags=re.MULTILINE
    )
