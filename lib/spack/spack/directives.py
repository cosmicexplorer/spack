...
@directive(dicts=("versions", "version_infos"))
def version(
    ver: Union[str, int],
...
class VersionReferenceInfo:
    ...
    def process_version(self) -> None:
        ...
        if self.commit is not None:
            if isinstance(cur_version, GitVersion) and cur_version.commit_sha != self.commit:
                raise VersionChecksumError(
                    f"{cur_version.commit_sha}: checksum provided twice, "
                    f"but did not match {self.commit}"
                )
            cur_version = GitVersion(f"git.{self.commit}={cur_version}")

        if self.tag is not None:
            if isinstance(cur_version, GitVersion):
                if cur_version.is_commit:
                    tty.debug(f"dropping tag {self.tag!r} to version name for commit {cur_version!r}")
                else:
                    raise VersionError(
        ...
        if self.branch is not None:
            if isinstance(cur_version, GitVersion):
                if cur_version.ref == self.branch:
                    ...
                    raise VersionError(
                        f"cannot figure out how to merge branch kwarg {self.branch!r} "
...
def _execute_version(pkg: Type[spack.package_base.PackageBase], ver: Union[str, int], **kwargs: Any) -> None:
    if (
