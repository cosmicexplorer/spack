...
def test_git_url_top_level_git_versions(mock_packages, config):
    ...
    # This raises because both tag= and commit= are specified!
    with pytest.raises(spack.fetch_strategy.FetcherConflict):
        spack.fetch_strategy.for_package_version(pkg, '3.1')
    ...
    # This raises because both branch= and commit= are specified!
    with pytest.raises(spack.fetch_strategy.FetcherConflict):
        spack.fetch_strategy.for_package_version(pkg, '3.3')
    ...
    assert '10-40 character hexadecimal string' in str(exc_info.value)
